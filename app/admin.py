...
class AppUpdateAdmin(ModelView, model=AppUpdate):
    ...
    Две ловушки, обе из жизни (19 сентября 2026). Первая: сравнивается номер
    из сборки, а не из App Store Connect — там выпуск назвали 1.1.0, а бинарник
    отвечает 1.7.1; порог 1.1.x ничего бы не сделал, порог выше линейки 1.7.x
    запер бы всех. Вторая: проверка появилась в коде 5 сентября и впервые
    уехала в магазин 17-го; iOS 1.0 от 25 августа сервер не спрашивает
    вообще, и никакой порог до неё не дотянется.
    """
    ...
    form_args = {
        "min_version": {
            "description": (
                "Сравнивается номер, который приложение сообщает о себе: "
                "CFBundleShortVersionString на iOS и versionName на Android — "
                "а не название выпуска в App Store Connect. Сейчас выпуск в магазине "
                "подписан 1.1.0, а сама сборка отвечает 1.7.1. Какие номера реально "
                "стоят у людей — раздел «Устройства с пушами», колонка «Версия». "
                "Сборки старше 17 сентября 2026 (iOS 1.0) порог не спрашивают вовсе: "
                "до них он не дотянется, обновятся только через магазин."
            )
        },
