...
def _needs(place: Place) -> list[str]:
    """Чего не хватает месту, чтобы его показывать людям.

    Сезона здесь нет намеренно. Он не заполнен у большинства каталога,
    включая давно опубликованные места, — значит это не порог публикации,
    а пожелание. Пока он считался пропуском, готовые места висели
    в черновиках из-за поля, которого нет и у соседей по списку.
    """
    out = []
    if not place.photos:
        out.append("фото")
